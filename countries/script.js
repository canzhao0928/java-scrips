...
const renderCountry = function (data, className = "") {
    const [key] = Object.keys(data.currencies)
    const { [key]: { name: name } } = data.currencies;
...
const renderErr = function (err) {
    countriesContainer.insertAdjacentText("beforeEnd", `There's something wrong...${err}. Try again! `)
}
...
// wait(2)
//     .then(() => {
//         console.log("2 secondes delay");
//         return wait(1)
//     })
//     .then(() => { console.log("1 secondes delay") })
...
const loadNPause = async function () {
    try {
        ...
        await createImage("./img/img-4.jpg")
    } catch (err) { console.error(err); }
}
